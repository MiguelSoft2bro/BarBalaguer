$(window).on("load", function() {
    ...
    var dataId = $('.list-menu li:first-child a').attr('data-id');
        ...
        success: function(data) {
          ...
          $("#contenedorLista").empty();
            ...
            for (let index = 0; index < datos.length; index++) {
                articulo = datos[index];
                

                item = '<div class="single-menu-item" style="margin: 30px 0px 40px 0px">';
                item +='<div class="single-menu-item-content">';
                item +='<h3>'+articulo["Articulo"]+'</h3>';
                item +='<p class="price">PRECIO: '+parseFloat(articulo["PVP"]).toFixed(2)+' €</p>';
                item +='</div>';
                item +='</div>';
